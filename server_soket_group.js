...
MongoClient.connect(process.env.MYDBURL, {useUnifiedTopology:true}, function(에러, client){
  ...
  */
  ...
  // 뭘 한거냐면 그냥 간략하게 express를 이용해서 서버를 띄우던걸 http라는 nodejs 기본 라이브러리 + socket.io를 이용해서 띄운겁니다.
...
app.get('/', function(요청, 응답){
    ...
    // <% } else { %> 해당 값을 불러오지 못함 <% } %> -->
      ...
      return 응답.render('index.ejs', { loginSucceeded: '로그인 아직 안함 / 세션아니라서 새로고침시 에러남'} );
  });
...
app.get('/list', function(요청, 응답){
  ...
  db.collection('postCol').find().toArray(function(에러, 결과){
    ...
    return 응답.render('list.ejs', { postResult : 결과, errorMessage: '' });
  });   
...
function 로그인했는지검사하는미들웨어(요청, 응답, next){
  ...
  if(요청.user){
    ...
    // 주의: 코드 리팩토링 방법 터득요. login.ejs와 loginrequired.ejs는 errorMessage 오브젝트 전달 유무와 form action 경로 차이. 동일 코드를 2개의 페이지로 중복운영.
    return 응답.render('loginrequired.ejs', { errorMessage: '해당 페이지는 로그인이 필요합니다'})
  }
}
...
app.post('/login', passport.authenticate('local', { failureRedirect: '/fail'}), function(요청, 응답){
  ...
  // 응답.redirect('/');
  응답.render('index.ejs', { loginSucceeded: '로그인 성공함 / 세션아니라서 새로고침시 에러남'})
...
app.post('/write', function(요청, 응답){
  ...
  db.collection('counterCol').findOne({name: '게시물갯수'}, function(에러, 결과){
    ...
    console.log(`app.post('/write') ---- 업로드요청한게시물 ----`, 업로드요청한게시물 )
...
// ★★★★★ 떠오른 과제: chatroomCol 에서 채팅방 삭제? 나가기? 기능 구현해야 함
// ★★★★★ 떠오른 과제: chatroomCol 에서 채팅방 삭제? 나가기? 기능 구현 후, 해당 채팅방에 귀속되었던(즉, messageCol 의 parent 값 = 즉, chatroomCol 의 _id 값) messageCol 에 들어있는 메세지들도 채팅방 삭제? 나가기? 시 메세지들도 연계 삭제?
...
app.post('/chatroom', 로그인했는지검사하는미들웨어, async function(요청, 응답){

  const 당한사람아이디 = ObjectId(요청.body.당한사람id);
  const 요청자아이디 = 요청.user._id;

  // const existingChatroom = await db.collection('chatroomCol').findOne( { member: [ 당한사람아이디, 요청자아이디 ]} )
  // console.log(`app.post('/chatroom', ) -------- existingChatroom ------- `, existingChatroom)

  if(String(당한사람아이디) === String(요청자아이디)){
    console.log(`app.post('/chatroom', ) --- 본인이 본인 채팅방으로 1인 2역하는 방 개설하려고 함`);
    // [ AJAX 문법 & Express 문법 ] The server cannot do a redirect from an ajax request. In the end ajax involves the client (browser).
    // [ AJAX 문법 & Express 문법 ] If you want, you can send the instruction of a redirection through the server side call, but it will be end up again on client side, in the callback. 
    // [ AJAX 문법 & Express 문법 ] You can do that by returning an object from the server which contains the url you want to redirect to. 
    // [ AJAX 문법 & Express 문법 ] Then use javascript to change the document's location property. Like Below:
    // return 응답.status(400).redirect('/list');
    return 응답.status(400).send({result: 'redirect', url:'/list'})
  };

  const existingChatroom111 = await db.collection('chatroomCol').findOne( { member: [ 당한사람아이디, 요청자아이디 ]} )
  const existingChatroom222 = await db.collection('chatroomCol').findOne( { member: [ 요청자아이디, 당한사람아이디 ]} )

  console.log(`findOne( { member: [ 당한사람아이디, 요청자아이디 ]} ) ---- existingChatroom111 --- `, existingChatroom111)
  console.log(`findOne( { member: [ 요청자아이디, 당한사람아이디 ]} ) ---- existingChatroom222 --- `, existingChatroom222)

  if(existingChatroom111 || existingChatroom222){
    console.log(`app.post('/chatroom', ) --- 한 곳에라도 이미 채팅방이 있음`) 
    // [ AJAX 문법 & Express 문법 ] The server cannot do a redirect from an ajax request. In the end ajax involves the client (browser).
    // [ AJAX 문법 & Express 문법 ] If you want, you can send the instruction of a redirection through the server side call, but it will be end up again on client side, in the callback. 
    // [ AJAX 문법 & Express 문법 ] You can do that by returning an object from the server which contains the url you want to redirect to. 
    // [ AJAX 문법 & Express 문법 ] Then use javascript to change the document's location property. Like Below:
    // return 응답.redirect('/chat')
    return 응답.status(200).send({result: 'redirect', url:'/chat'})
  } else {
    console.log(`app.post('/chatroom', ) --- 어느 곳에도 채팅방 없음`)
    ...
    let newChatroom = await db.collection('chatroomCol').insertOne(개설할채팅방정보)
      
    console.log(`app.post('/chatroom', ) --- 채팅방개설성공 --- ${요청.user._id} 사용자가 ${요청.body.당한사람id} 사용자에게`);
...
app.get('/chat', 로그인했는지검사하는미들웨어, async function(요청, 응답){
  ...
  let myChatroomsResult = await db.collection('chatroomCol').find({ member: 요청.user._id }).toArray()

  // ★★★★★★ 떠오른 과제: mychatroomsResult 에는 { member: [ 오브젝트형아이디 , 오브젝트형아이디] } 에 내 아이디, 상대방 아이디가 엎치락 뒤치락 섞여 있으므로 
  // chat.ejs 의 <span class="chat-box mine"></span> 의 mine 클래스를 적용하기 위한 '나'의 메세지들만 특정하기 위해 
  // 응답.render() 에 await db.collection('loginCol').find({ _id: 요청.user._id }) 로 '나' 의 아이디 값을 넘겨줘서 
  // myChatroomList[i].member[0] 와 myChatroomList[i].member[1] 중에 어찌됐든 '나'의 아이디 값이 있다면 mine 클래스 적용시켜서 채팅방의 네메세지, 내메세지 좌우로 CSS 적용되도록 구현요

  console.log(`app.get('/chat', ) -------- await db.collection('chatroomCol').find().toArray() ---- `, myChatroomsResult)
  
  return 응답.render('chat.ejs', { myChatroomList: myChatroomsResult });
});
...
// app.get('/message', 로그인했는지검사하는미들웨어, function(요청, 응답){
  ...
  console.log('요청.params.clickedChatroomId --------', 요청.params.clickedChatroomId)

///////////////////////////////////////////////////
// 응답.write('event: 서버에서작명한메시지보내기이벤트\n'); // 'event: 보낼데이터이름\n'
// 응답.write(`data: 되라되라\n\n`); // 'data: 보낼데이터\n\n'
///////////////////////////////////////////////////
    ...
    db.collection('messageCol').find({ parent: 요청.params.clickedChatroomId }).toArray().then(특정채팅방기존메시지내역 => { 
    ...
    console.log(`app.get('/message/:clickedChatroomId', ) ---- 특정채팅방기존메시지내역 ---- `, 특정채팅방기존메시지내역)
    ...
    // [ Web API 문법 & Mongo DB 문법^^ ] ★★★ 응답.write(`data: ${어쩌구}\n\n`) 에서 개행 \n\n 두 번 연속 기입하지 않고 한 번만(즉, \n) 기입하면 브라우저 콘솔에 안 찍히네?? 왜 반드시 \n\n 해야 하는건자?
    
    응답.write(`data: ${JSON.stringify(특정채팅방기존메시지내역)}\n\n`); // 'data: 보낼데이터\n\n'

    // ★★★★★★ 긴급 해결요 ★★★★★★★
    // [ MongoDB 문법^^ Change Stream ] DB변동사항 실시간 감시해 브라우저단에 표시해 주도록 만든 상태인 것은 좋은 일
    // [ MongoDB 문법^^ Change Stream - 221003 자체해결완 ] 그러나 chatroomCol 에 두 명의 사용자가 참여했음에도 유일한 한 개의 방이 아니라 당한사람 입장, 건사람 입장에서 각각 방을 생성시켜 동작하는 상황이라 해결요(다행인 것은 DB에도 브라우저에도 송신한 메시지들은 다 반영이 되는 상황)
    ...
    /* 
    // [ MongoDB 문법^^ Change Stream ] change stream 기법 생김새 중 일례 
    // const 찾을문서 = [
    ...
    */
    ...
    /*
    // // [ MongoDB 문법^^ Change Stream ] change stream 기법 생김새 from 코애
    // // db.collection('messageCol')
    // //   .watch([ 
    // //           { $match: { 
    // //               'fullDocument.parent': 요청.params.clickedChatroomId 
    // //               } 
    // //          } ])
    // //   .on('change', (result) => { 응답.write() })    
    */
      ...
      // [ MongoDB 문법^^ Change Stream ] ★★★ MongoDB Change Stream 기능을 활용해 DB 변동사항 발생시 즉시 서버에 알려주도록(change stream 기법) 만들어야 사용자에게 실시간 데이터 전송(서버단: SSE, Server Sent Event / 프론트엔드단: new EventSource() 와 eventSource.close() 등)이 실현될 수 있음    
    ...
    changeStream.on('change', (result) => {
      ...
      console.log(`app.get('/message/:clickedChatroomId', ) ---- CHANGE STREAM ---- `, result.fullDocument);
    ...
    //   // [ MongoDB 문법^^ Change Stream ] ★★★ 요 이슈 해결! -> 소통채널 접속시 일회성 검색 및 일회성 검색결과 전송하는 작업처리에 그쳐서 사용자가 새로 송신한 메시지(즉, class="chat-content" 내의 class="chat-box" 형태)가 브라우저 채팅창 화면에 보여지지 않음
      ...
      // 응답.render('/chat', { loggedInUserId: 요청.user._id })
...
app.post('/message', 로그인했는지검사하는미들웨어, async function(요청, 응답){
  ...
  console.log(`server.js ---- app.post('/message', ) --- 송신발저장용꾸러미 ----`, 송신발저장용꾸러미);

  try{
    await db.collection('messageCol').insertOne(송신발저장용꾸러미)
    console.log('messageCol 에 송신발저장용꾸러미 저장 성공');

    return 응답.status(200).send('messageCol 에 송신발저장용꾸러미 저장 성공');
  } catch(err) { 
    console.log(`server.js ---- db.collection('messageCol').insertOne() --- catch(err) ----`, err)
  }
});
...
// 기능1 - 특정 채팅방 생성 및 입장
  ...
  //  [ socket.io 문법** 양방향 중 서버->유저 ] socket.ejs 에서 발신한 socket.emit('emit-to-join-room1', ) 을 서버의 socket.on('emit-to-join-room1', ) 으로 받아서 socket.join('방이름') 통해서 채팅방 생성하고 입장시킴
  socket.on('emit-to-join-room1', function(reqUserSent){
    // [ socket.io 문법** ] socket.join('방이름') 코드로 채팅방 생성하고 입장시키기
    socket.join('joined-room1');
  });




  // 기능2 - 특정 채팅방 참여자간 broadcast



  // [ socket.io 문법** 양방향 중 유저->서버 ] HTML 단에서 socket.emit('이벤트명작명','메시지') 코드로 서버에 실시간 메시지 전송함
  // 현재 코드 단계에서는 room1 외부인도 room1에 메시지 보내기 버튼 누르면 room1에 입장하지 않은 상태임에도 room1 에 있는 사람들에게 메시지 송신 가능함(즉, room1 입장여부 체크해서 room1 메시지 송신 기능 허용해야 함)
  socket.on('room1-send', function(dataUserSentForRoom1){
    // [ socket.io 문법** ] io.to('joined-room1').emit(); 코드로 특정 채팅방(여기서는 joined-room1)에 들어간 사용자들에게만 메시지 전송됨
    io
      .to('joined-room1')
      .emit('broadcast', dataUserSentForRoom1);
  });
  ...
  socket.on('user-send', function(dataUserSent){
    ...
    // 기능3 - 서버와 특정 유저 1명 간 단독 소통(즉, 개별 사용자와 서버 간 1:1 소통)
    ...
    //   console.log(`socket.on('user-send', function(dataUserSent){}) ---- socket.id ------- `, socket.id)
    //////////////////////////////////////
    /////////// 양자 택일 ////////////////
    /////////////////////////////////////
    //   io
    //     .to(socket.id)
    //     .emit('broadcast', dataUserSent);



  // 기능4 - 메세지 전송하면 모든 사용자에게 broadcast
